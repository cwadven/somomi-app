...
- 만료 판단: 템플릿의 `feature.validWhile` 정책을 최우선으로 평가합니다.
  ...
  - 주의: 현재 로직은 직접적인 `feature.expiresAt` 접근을 사용하지 않습니다. 고정 만료는 반드시 `feature.validWhile.expiresAt`로 표현합니다.
...
- 만료된 템플릿이 연결된 영역은 "영역 정보" 편집이 제한되며, 템플릿 변경/제품 슬롯만 허용
 - 뒤로가기 보호: 제목/설명 등 입력 또는 제품 슬롯 스테이징 변경이 있으면 뒤로가기 시 확인 모달로 이탈을 방지
...
- 폼 초안 자동 저장/복원(앱 재진입 시 복원, 동일 세션 뒤로가기 시 초기화)
 - 뒤로가기 보호: 작성된 내용이 있을 때 뒤로가기/제스처 시 "저장하지 않고 나가기" 확인 모달 표시

### C) 제품 상세/소진 처리 (`ProductDetailScreen`)
- 소진 처리 시 날짜 선택 모달 표시(연/월/일 중앙 정렬), 처리 후 `processedAt` 기록
- 만료된 템플릿 연결 영역은 소진/삭제 버튼 비활성(안내 모달 제공)

### D) 소진 목록 (`ConsumedProductsScreen`)
- 정렬 기준: "소진 처리" 시점만 사용(기본: 내림차순). 칩을 누르면 내림 → 오름 → 정렬 해제 순으로 순환
- 기준 필드: `consumedAt` 우선, 없으면 `processedAt`. 값이 없는 항목은 리스트 하단으로 이동
...
- (그 외) 알림/카테고리/처리 로그 등
 - 알림 관련
   - `somomi_processed_notifications`: 날짜별로 처리/전송된 알림 스냅샷
   - `somomi_daily_reminder_sent`: 9시 리마인더 발송/예약 여부(일자 키)
   - `somomi_daily_update_reminder_sent`: 20시 작성 리마인더 발송/예약 여부(일자 키)
...
- 9시/20시 리마인더 스케줄, 디버그 즉시 발송 시에도 비활성/만료 영역/제품을 필터링
 - 중복 방지: 매일 알림은 고정 ID(`daily_09_00`, `daily_20_00`)로 예약하고, 재등록 시 동일 ID의 기존 트리거를 취소 후 재예약하여 항상 1개만 활성 상태를 유지합니다. 또한 하루 1회 발송 기록을 저장해 중복 발송을 방지합니다.
